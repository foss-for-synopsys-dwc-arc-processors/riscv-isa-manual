...
[[base32_instructions_rr, reftext="Base 32 bits version Instructions"]]
=== Instructions
...
Description:: 
The ADD instruction performs register-to-register addition. It adds the value from the first source register (rs1) to the value from the seconde source register (rs2) and then stores the result in the destination register (rd).
...
Description:: 
Adds the sign-extended 12-bit immediate to the value in the first source register (rs1) and stores the result in the destination register (rd). This instruction is commonly used for small constant additions and to implement register-register moves.
...
Description::
Performs a bitwise AND operation on the value of the first source register (rs1) to the value from the second source register (rs2) and then stores the result in de destination register (rd).
Each bit of the result is set to 1 if both corresponding bits of the first and second operands are 1, otherwise, it is set to 0.
...
Description:: 
Performs a bitwise AND operation between the value in the first source register (rs1) and the sign-extended 12-bit immediate. The result is stored in the destination register (rd). This instruction is useful for masking specific bits in a register.
...
Description:: 
Forms a 32-bit offset from the 20-bit U-immediate, filling in the lowest 12 bits with zeros, adds this offset to the address of the AUIPC instruction, then places the result in the destination register (rd). This instruction is useful for PC-relative addressing.
...
Description:: 
Compares the values in two registers (rs1 and rs2). If they are equal, the program counter is set to the current PC plus the sign-extended 12-bit offset (multiplied by 2). This creates a conditional branch with a range of ±4 KiB.
...
Description:: 
Compares the values in two registers (rs1 and rs2) as signed integers. If rs1 is greater than or equal to rs2, the program counter is set to the current PC plus the sign-extended 12-bit offset (multiplied by 2).
...
Description:: 
Compares the values in two registers (rs1 and rs2) as unsigned integers. If rs1 is greater than or equal to rs2, the program counter is set to the current PC plus the sign-extended 12-bit offset (multiplied by 2).
...
Description:: 
Compares the values in two registers (rs1 and rs2) as signed integers. If rs1 is less than rs2, the program counter is set to the current PC plus the sign-extended 12-bit offset (multiplied by 2).
...
Description:: 
Compares the values in two registers (rs1 and rs2) as unsigned integers. If rs1 is less than rs2, the program counter is set to the current PC plus the sign-extended 12-bit offset (multiplied by 2).
...
Description:: 
Compares the values in two registers (rs1 and rs2). If they are not equal, the program counter is set to the current PC plus the sign-extended 12-bit offset (multiplied by 2). This creates a conditional branch with a range of ±4 KiB.
...
Description:: 
Raises a breakpoint exception, transferring control to a debugging environment. This is used to implement breakpoints in debuggers or for system software debugging.
...
Description:: 
Generates an environment call exception, which transfers control to the operating system or execution environment. This is used to make system calls or request services from the execution environment.
...
Description:: 
Used to enforce ordering between memory operations and I/O. It ensures that the effects of instructions before the fence are visible before the effects of instructions after the fence. This is crucial for memory consistency in multiprocessor systems and for synchronizing with memory-mapped I/O.
...
Description:: 
Use this instruction to jump to the new program counter address.
Jump instruction can target any address who the 20 offset bits add with actual PC value can reach. The actual pc value will be stored in a register (rd) for future use.
...
Synopsis:: 
Unconditional jump and link register
...
Description:: 
Use this instruction to jump to the new program counter address.
The immediate value will be added to a register value (rs1) and that will be the new promgram counter address. The actual pc value will be stored in a register (rd) for future use.
...
Description:: 
Loads an 8-bit value from memory, sign-extends it to 32 bits, and writes it to register rd. The effective address is obtained by adding register rs1 to the sign-extended 12-bit offset.
...
Description:: 
Loads an 8-bit value from memory, zero-extends it to 32 bits, and writes it to register rd. The effective address is obtained by adding register rs1 to the sign-extended 12-bit offset.
...
Description:: 
Loads a 16-bit value from memory, sign-extends it to 32 bits, and writes it to register rd. The effective address is obtained by adding register rs1 to the sign-extended 12-bit offset.
...
Description:: 
Loads a 16-bit value from memory, zero-extends it to 32 bits, and writes it to register rd. The effective address is obtained by adding register rs1 to the sign-extended 12-bit offset.
...
Description:: 
Loads the 20-bit immediate value into the upper 20 bits of register rd, filling in the lower 12 bits with zeros. This instruction is used to build 32-bit constants.
...
Description:: 
Loads a 32-bit value from memory and writes it to register rd. The effective address is obtained by adding register rs1 to the sign-extended 12-bit offset.
...
Description:: 
Performs a bitwise OR operation on the value of the first source register (rs1) to the value from the second source register (rs2) and then stores the result in de destination register (rd).
Each bit of the result is set to 1 if one of the bits corresponding to the first and second operands is 1. Only if both bits are zero, then the result is set to 0.
...
Description:: 
Performs a bitwise OR operation between the value in the first source register (rs1) and the sign-extended 12-bit immediate. The result is stored in the destination register (rd). This instruction is useful for setting specific bits in a register.
...
Description:: 
Stores the lowest 8 bits from register rs2 to memory. The effective address is obtained by adding register rs1 to the sign-extended 12-bit offset.
...
Description:: 
Stores the lowest 16 bits from register rs2 to memory. The effective address is obtained by adding register rs1 to the sign-extended 12-bit offset.
...
Description:: 
Compares the value in register rs1 with the sign-extended 12-bit immediate as signed integers. If rs1 is less than the immediate, the instruction writes 1 to rd; otherwise, it writes 0.
...
Description:: 
Compares the value in register rs1 with the sign-extended 12-bit immediate as unsigned integers. If rs1 is less than the immediate, the instruction writes 1 to rd; otherwise, it writes 0.
...
Description:: 
SLTU performs unsigned comparisons, writing 1 to the destination register (rd) if rs1 is less than rs2 (unsigned), 0 otherwise.
...
Description:: 
Subtract the value of the first source register (rs1) to the value from the second source register (rs2) and then stores the result in de destination register (rd).
...
Description:: 
Logical bitwise exclusive OR of source operand 1 (rs1) with source operand 2 (rs2). The result is written into the destination register (rd). 
If the bits are equal the output is zero and when they are different the output is one.
